...
def generar_mensaje_resumen(hora_str=None):
    """
    Genera el texto del resumen para WhatsApp.
    - CHECK-IN HOY:       fecha_entrada == hoy
    - PRÓXIMAS ENTRADAS:  fecha_entrada en los próximos 7 días
    - CHECK-OUT HOY:      fecha_salida == hoy
    """
    hoy = date.today()
    if hora_str is None:
    ...
    for r in reservas:
        ...
        if fe == hoy:
            checkin_hoy.append(r)
        elif hoy < fe <= hoy + timedelta(days=7):
            proximas.append(r)

        if fs == hoy:
            checkout_hoy.append(r)

    # Ordenar próximas por fecha de entrada
    proximas.sort(key=lambda x: x["fecha_entrada"])

    hoy_fmt = hoy.strftime("%d/%m/%Y")
    lineas = [f"🏨 ALCHOMES — {hoy_fmt} · {hora_str}:00h"]

    lineas.append("\n✅ CHECK-IN HOY (parte rellenado):")
    if checkin_hoy:
        for r in checkin_hoy:
            lineas.append(f"• {r['habitacion']} — {r['email'] or 'sin email'}")
    else:
        lineas.append("• (ninguno registrado)")

    lineas.append("\n📅 PRÓXIMAS ENTRADAS (parte ya rellenado):")
    if proximas:
        for r in proximas:
            fe = date.fromisoformat(r["fecha_entrada"])
            lineas.append(f"• {r['habitacion']} — {fe.strftime('%d/%m')} → {r['email'] or 'sin email'}")
    else:
        lineas.append("• (ninguna próxima)")

    lineas.append("\n🚪 CHECK-OUT HOY:")
    if checkout_hoy:
        for r in checkout_hoy:
            lineas.append(f"• {r['habitacion']}")
    else:
...
def enviar_whatsapp_callmebot(mensaje):
    """
    Envía un mensaje WhatsApp via CallMeBot.
    Requiere variables CALLMEBOT_PHONE y CALLMEBOT_API_KEY en Render.
    """
    ...
    if resp.status_code != 200:
        raise Exception(f"CallMeBot respondió {resp.status_code}: {resp.text[:300]}")
    logger.info(f"CallMeBot OK: {resp.text[:120]}")
    return resp.text
